...
class BotConfigurationService:
    ...
    CATEGORY_DESCRIPTIONS: dict[str, str] = {
        ...
        'METERED_TRAFFIC': ('Лимит действует только для выделенного squad. Остальные серверы остаются безлимитными.'),
    ...
    CATEGORY_KEY_OVERRIDES: dict[str, str] = {
        ...
        'ULTIMA_METERED_NODE_MULTIPLIERS': 'METERED_TRAFFIC',
    ...
    CHOICES: dict[str, list[ChoiceOption]] = {
        ...
        'TAP_REWARDS_REWARD_TYPE': [
            ChoiceOption('subscription_days', '📅 Дни к подписке'),
            ChoiceOption('balance', '💰 Баланс'),
        ],
    }
    ...
    SETTING_HINTS: dict[str, dict[str, str]] = {
        ...
        'TAP_REWARDS_ENABLED': {
            'description': 'Включает подарки за нажатия по логотипу на главном экране Ultima.',
            'format': 'Булево значение.',
            'example': 'false',
            'warning': 'Перед включением настройте порог, тип подарка и дневной лимит выдач.',
        },
        'TAP_REWARDS_THRESHOLD': {
            'description': 'Сколько тапов нужно накопить пользователю для одного подарка.',
            'format': 'Целое число от 1 и выше.',
            'example': '100',
            'warning': 'Слишком низкий порог может быстро расходовать баланс или дни подписки.',
            'dependencies': 'TAP_REWARDS_ENABLED',
        },
        'TAP_REWARDS_REWARD_TYPE': {
            'description': 'Что выдавать пользователю после достижения порога тапов.',
            'format': 'Выберите один из вариантов: дни к подписке или баланс.',
            'example': 'subscription_days',
            'dependencies': 'TAP_REWARDS_ENABLED',
        },
        'TAP_REWARDS_SUBSCRIPTION_DAYS': {
            'description': 'Сколько дней подписки начислять за один подарок.',
            'format': 'Целое число дней. Используется, если выбран тип подарка «Дни к подписке».',
            'example': '1',
            'dependencies': 'TAP_REWARDS_REWARD_TYPE=subscription_days',
        },
        'TAP_REWARDS_BALANCE_KOPEKS': {
            'description': 'Сколько копеек начислять на баланс за один подарок.',
            'format': 'Сумма в копейках.',
            'example': '5000 = 50 ₽',
            'dependencies': 'TAP_REWARDS_REWARD_TYPE=balance',
        },
        'TAP_REWARDS_DAILY_REWARD_LIMIT': {
            'description': 'Сколько подарков максимум можно выдать одному пользователю за сутки.',
            'format': 'Целое число. 0 отключает дневной лимит.',
            'example': '1',
        ...
        'ULTIMA_METERED_NODE_UUIDS': {
            ...
            'warning': 'Коэффициенты задаются на отдельной странице раздельного трафика.',
        },
        'ULTIMA_METERED_NODE_MULTIPLIERS': {
            'description': 'Карта UUID нод и коэффициентов учета трафика.',
            'format': 'JSON-объект. Значения от 0.1 до 100 с точностью до одного знака.',
            'example': '{"uuid-ноды-1":1,"uuid-ноды-2":2}',
